...
public class Storage2QueryProcessor extends org.fiap.common.StorageQueryProcessor implements IDbCommand {
	...
	java.sql.PreparedStatement m_PreparedStatement=null;
...
MAIN : while(m_KeyIndex<m_Keys.length){
				 ...
				 if(m_CursorName==null){
					 ...
					 m_PreparedStatement=connection.prepareStatement(sql);
					 m_PreparedStatement.setString(1, id);

					 ResultSet rset=m_PreparedStatement.executeQuery();
					 if(rset.next()){
					 ...
					 if(!ispoint){
						 ...
						 sql="SELECT id,parent,ispoint FROM pointsettree WHERE parent=?";
						 m_PreparedStatement=connection.prepareStatement(sql);
						 m_PreparedStatement.setString(1, id);

						 rset=m_PreparedStatement.executeQuery();
						 while(rset.next()){
						 ...
						 if(select!=null || eq!=null){
							 ...
							 m_PreparedStatement=connection.prepareStatement(sql);
							 setWHERE(1, id,eq,neq,lt,gt,lteq,gteq,select);					
	...
	private String createWHERE(String id, String eq, String neq, String lt, String gt, String lteq, String gteq, String select){
		...
		sb.append("WHERE id=?");
		if(eq!=null){
			sb.append(" AND time=?::Timestamp");
		}
		if(neq!=null){
			sb.append(" AND NOT time=?::Timestamp");
		}
		if(lt!=null){
			sb.append(" AND time<?::Timestamp");
		}
	...
	private void setWHERE(int offset, String id, String eq, String neq, String lt, String gt, String lteq, String gteq, String select) throws SQLException {
		int index = 0;
		m_PreparedStatement.setString(offset+index, id);
		index++;
		if(eq!=null){
			m_PreparedStatement.setString(offset+index, eq.replaceAll("T"," "));
			index++;
		}
		if(neq!=null){
			m_PreparedStatement.setString(offset+index, neq.replaceAll("T"," "));
			index++;
		}
		if(lt!=null){
			m_PreparedStatement.setString(offset+index, lt.replaceAll("T"," "));
			index++;
		}
		if(gt!=null){
			m_PreparedStatement.setString(offset+index, gt.replaceAll("T"," "));
			index++;
		}
		if(lteq!=null){
			m_PreparedStatement.setString(offset+index, lteq.replaceAll("T"," "));
			index++;
		}
		if(gteq!=null){
			m_PreparedStatement.setString(offset+index, gteq.replaceAll("T"," "));
			index++;
		}
  }
}
